...
services:
  $pc1_name:
    ...
    command: bash -c "apt update && apt install -y iputils-ping iproute2 net-tools ipcalc traceroute nmap && tail -f /dev/null"
    networks:
      $flat_net:
...
case "$1" in
    ...
    *)
        show_banner
        echo "Usage: $0 -start [kali|ubuntu] | -stop | -clean | -run [pc1|pc2|pc3|pc4|pc5|pc6] | -restart"
        echo ""
