...
services:
  $v1_name:
    ...
    command: ["bash", "/Setups/Server01.sh"]
    cap_add:
      - NET_ADMIN
      - NET_RAW
    networks:
  ...
  $v2_name:
    ...
    command: ["bash", "/Setups/Server02.sh"]
    cap_add:
      - NET_ADMIN
      - NET_RAW
    networks:
  ...
  $v3_name:
    ...
    command: ["bash", "/Setups/Server03.sh"]
    cap_add:
      - NET_ADMIN
      - NET_RAW
    networks:
  ...
  $attacker_name:
    ...
    command: bash -c "apt update && apt install -y nmap tcpdump iproute2 net-tools iputils-ping && tail -f /dev/null"
    cap_add:
      - NET_ADMIN
      - NET_RAW
    networks:
